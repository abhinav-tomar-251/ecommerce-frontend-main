...
<aside className='bg-white min-h-full  w-full  max-w-60 customShadow pt-28'>
        <div className='h-32  flex justify-center items-center flex-col'>
            <div className='text-5xl cursor-pointer relative flex justify-center'>
                {
                user?.profilePic ? (
                    <Image src={user?.profilePic} width={80} height={80} className=' rounded-full' alt={user?.name} />
                ) : (
                    <FaRegCircleUser/>
                )
                }
        ...
        <div>   
            <nav className='grid p-4'>
                <Link href={"/admin/AllUsers"} className='px-2 py-1 hover:bg-slate-100'>All Users</Link>
                <Link href={"/admin/AllProducts"} className='px-2 py-1 hover:bg-slate-100'>All product</Link>
            </nav>
        </div>  
</aside>
