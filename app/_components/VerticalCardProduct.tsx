...
const VerticalCardProduct: React.FC<VerticalCardProductProps> = ({ category, heading }) => {
    ...
    const handleAddToCart = async (e: React.MouseEvent<HTMLButtonElement>, _id: string) => {
        ...
        <div className='container mx-auto px-4 my-6 relative'>
            ...
            <div className='flex items-center gap-4 md:gap-6 overflow-x-scroll scrollbar-none transition-all' ref={scrollElement}>
                ...
                {loading ? (
                    ...
                    data.map((product) => (
                        <Link key={product?._id} href={`product/${product?._id}`} className='w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-sm shadow'>
                            ...
                            <div className='p-4 grid gap-3'>
                                ...
                                <button className='text-sm bg-gray-600 hover:bg-gray-700 text-white px-3 py-0.5 rounded-full' onClick={(e) => handleAddToCart(e, product?._id)}>
                                    Add to Cart
                                </button>
                            </div>
                        </Link>
